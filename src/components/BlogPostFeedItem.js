              ...
              <div className="card">
                ...
                <div className="card__body">
                  ...
                  <footer className="card__footer">
                    <span className="text-gray-200 font-display">On <time dateTime={moment(_.get(post, 'frontmatter.date', null)).strftime('%Y-%m-%d %H:%M')}>{moment(_.get(post, 'frontmatter.date', null)).strftime('%B %d, %Y')}</time></span>
                    {_.get(post, 'frontmatter.author', null) && ((() => {
                        let author = _.get(post, 'frontmatter.author', null);
                        return (
                          author.link ? (
                            <span className="text-gray-200 font-display"> by <Link to={withPrefix(author.link)}>{author.first_name} {author.last_name}</Link></span>
                          ) : 
                            <span className="text-gray-200 font-display"> by {author.first_name} {author.last_name}</span>
                        );
                    })())}
                  </footer>
                </div>
