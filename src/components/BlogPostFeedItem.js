                ...
                <div className="card__body">
                  <header className="card__header">
                    ...
                    {_.get(blog_feed_section, 'title', null) ? (
                      ...
                      <h2 className="text-2xl card__title text-gray-vlight font-display"><Link to={withPrefix(_.get(post, 'url', null))}>{_.get(post, 'frontmatter.title', null)}</Link></h2>
                    }
                  </header>
                  {_.get(post, 'frontmatter.excerpt', null) && (
                    <div className="card__copy">
                      <p className="text-gray-light font-display">{_.get(post, 'frontmatter.excerpt', null)}</p>
                    </div>
                  )}
                  <footer className="card__footer">
                    <span className="text-gray-normal font-display">On <time dateTime={moment(_.get(post, 'frontmatter.date', null)).strftime('%Y-%m-%d %H:%M')}>{moment(_.get(post, 'frontmatter.date', null)).strftime('%B %d, %Y')}</time></span>
